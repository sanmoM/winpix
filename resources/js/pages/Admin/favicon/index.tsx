...
export default function SeriesIndex({
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <ToastContainer />
            <Head title={t('dashboard.favicon.index.title')} />

            <TableContainer>
                <TableTopSection
                    href={route('admin.favicon.create')}
                    title={t('dashboard.favicon.index.title')}
                    hasCreateButton={!favicons?.length}
                />

                <Table
                    headingItems={t('dashboard.favicon.index.table.headings', {
                        returnObjects: true,
                    })}
                >
                    {favicons?.length > 0 ? (
                        favicons.map((item, index) => (
